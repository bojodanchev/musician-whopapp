...
function simulateCreditDecrement(current: number, batch: number) {
  const b = Math.max(1, Number(batch) || 1);
  return Math.max(0, current - b);
}

type TestResult = { name: string; pass: boolean; got: unknown; expected: unknown };
...
export default function MusicianApp() {
  ...
  function runTests() {
    ...
    results.push({ name: "clamp low", pass: clamp(10, 40, 220) === 40, got: clamp(10, 40, 220), expected: 40 });
    results.push({ name: "clamp high", pass: clamp(300, 40, 220) === 220, got: clamp(300, 40, 220), expected: 220 });
    results.push({ name: "credit decrement basic", pass: simulateCreditDecrement(10, 3) === 7, got: simulateCreditDecrement(10, 3), expected: 7 });
    results.push({ name: "credit never below zero", pass: simulateCreditDecrement(2, 10) === 0, got: simulateCreditDecrement(2, 10), expected: 0 });
    results.push({ name: "credit invalid batch -> 1", pass: simulateCreditDecrement(5, 0) === 4, got: simulateCreditDecrement(5, 0), expected: 4 });
    results.push({ name: "presets >= 3", pass: presets.length >= 3, got: presets.length, expected: ">=3" });
        ...
        <div className="mx-auto max-w-6xl px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              {name:"Pro", price:"$79/mo", features:["600 credits","Stems","Batch x10"]},
              {name:"Studio", price:"$199/mo", features:["2000 credits","Team seats","Webhook/API"]},
            ].map((p)=> (
